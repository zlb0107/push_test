...
import (
	...
	"git.inke.cn/BackendPlatform/jaeger-client-go/internal/baggage"
	"git.inke.cn/BackendPlatform/jaeger-client-go/internal/throttler"
	"git.inke.cn/BackendPlatform/jaeger-client-go/log"
	"git.inke.cn/BackendPlatform/jaeger-client-go/utils"
	"git.inke.cn/BackendPlatform/jaeger-client-go/gid"
)
...
type Tracer struct {
	...
	options struct {
		...
		highTraceIDGenerator func(v []byte) uint64 // custom high trace ID generator
...
func NewTracer(
	...
	t.options.gen128Bit = true
	if t.options.gen128Bit {
		if t.options.highTraceIDGenerator == nil {
			t.options.highTraceIDGenerator = gid.Fnv32aExt
		}
...
func (t *Tracer) StartSpan(
	...
	return t.startSpanWithOptions(operationName, 0, sso)
}

func (t *Tracer) StartSpanWithTraceID(
	operationName string,
	traceID uint64,
	options ...opentracing.StartSpanOption,
) opentracing.Span {
	...
	return t.startSpanWithOptions(operationName, traceID, sso)
}
...
func (t *Tracer) startSpanWithOptions(
	...
	if !hasParent || !parent.IsValid() {
		...
		if t.options.gen128Bit {
			ctx.traceID.High = t.options.highTraceIDGenerator([]byte(t.serviceName + ":" + operationName))
		}
