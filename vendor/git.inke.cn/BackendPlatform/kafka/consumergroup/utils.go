...
func dividePartitionsBetweenConsumers(consumers kazoo.ConsumergroupInstanceList, partitions kazoo.PartitionList) map[string][]*kazoo.Partition {
	...
	n := plen / clen // numPartitionsPerConsumer
	m := plen % clen //consumersWithExtraPartition
	p := 0
	for i, consumer := range consumers {
		...
		for _, pl := range partitions[first:last] {
			result[consumer.ID] = append(result[consumer.ID], pl)
		}
		p = last
	}
