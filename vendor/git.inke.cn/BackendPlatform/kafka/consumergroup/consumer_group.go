...
type ConsumerGroup struct {
	...
	consumers map[string]kazoo.ConsumergroupInstanceList
...
TOPICS_CONSUMER:
	for {
		...
		if err != nil {
			cg.Logf("FAILED to get list of registered consumer instances: %s, try after 1s\n", err)
			time.Sleep(1 * time.Second)
			continue TOPICS_CONSUMER
		}
		topicConsumers := make(map[string]kazoo.ConsumergroupInstanceList)
		ownOccured := false
		for _, consumer := range consumers {
			if consumer.ID == cg.instance.ID {
				ownOccured = true
			}
			reg, err := consumer.Registration()
			if err != nil {
				cg.Logf("FAILED to get instance registered info : %s, try after 1s\n", err)
				time.Sleep(1 * time.Second)
				continue TOPICS_CONSUMER
			}
			for topic := range reg.Subscription {
				if topicConsumers[topic] == nil {
					topicConsumers[topic] = make(kazoo.ConsumergroupInstanceList, 0)
				}
				topicConsumers[topic] = append(topicConsumers[topic], consumer)
			}
		}
		if !ownOccured {
			cg.Logf("FAILED our instance ID not occured in topic consumers, maybe my session is expired, try to reregister!\n")
			cg.ensureRegistered(topics)
			continue TOPICS_CONSUMER
		}

		cg.consumers = topicConsumers
		for _, t := range topics {
			debugData := "["
			for i, c := range cg.consumers[t] {
				debugData = debugData + c.ID
				if i != len(cg.consumers[t])-1 {
					debugData = debugData + ","
				}
			}
			debugData = debugData + "]"
			cg.Logf("Currently registered topic %s consumers: %d, %s\n", t, len(cg.consumers[t]), debugData)
			if len(cg.consumers[t]) == 0 {
				cg.Logf("FAILED to get topic %s consumers %s size: %d, unexpected size, try after 1s\n", t, cg.group.Name, len(cg.consumers[t]))
				cg.ensureRegistered(topics)
				continue TOPICS_CONSUMER
			}
		}
...
func (cg *ConsumerGroup) ensureRegistered(topics []string) {
...
func (cg *ConsumerGroup) topicConsumer(topic string, messages chan<- *sarama.ConsumerMessage, errors chan<- error, stopper <-chan struct{}) {
	...
	defer cg.Logf("%s :: Stopped topic consumer\n", topic)

	// Fetch a list of partition IDs
	// partitions, err := cg.kazoo.Topic(topic).Partitions()
	for {

		partitions, partitionChanged, err := cg.kazoo.Topic(topic).WatchPartitions()
		if err != nil {
		...
		dividedPartitions := dividePartitionsBetweenConsumers(cg.consumers[topic], partitions)
		...
		for _, pid := range myPartitions {
			...
			wg.Add(1)
			go cg.partitionConsumer(topic, pid.ID, messages, errors, &wg, st)
		}
		select {
		case <-stopper:
			close(st)
		...
		cg.Logf("%s :: Triggering rebalance due to partition list change\n", topic)
	}
