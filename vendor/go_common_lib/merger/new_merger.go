...
import (
	...
	"go_common_lib/random_list"
)

type NewMergeStruct struct {
	Length   int     //队列长度
	Idx      int     //队列现偏移
...
func (this *MergeWrapper) Merge(Merge_map map[string]NewMergeStruct) string {
	if this.List == nil {
		this.List = random_list.RandomList.GetRandomList()
	}
	...
	randnum := float64(this.List[this.Idx]) / 101.0
	this.Idx++
	//寻找命中区间，左闭右开
	for key, ms := range Merge_map {
