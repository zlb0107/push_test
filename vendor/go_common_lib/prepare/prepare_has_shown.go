...
type HasShownPrepare struct {
}

func init() {
	var rp HasShownPrepare
	Prepare_map["HasShownPrepare"] = rp
	logs.Warn("in raw_prepare init")
	//cache.Cache_controlor.Register("rec_filter_")
}
func (rp HasShownPrepare) Get_data(request *data_type.Request, c chan string) int {
	defer func() { c <- "HasShownPrepare" }()
	//	defer common.Timer("HasShownPrepare", &(request.Timer_log), time.Now())
	is_hit := false
	defer common.TimerV2("HasShownPrepare", &(request.Timer_log), time.Now(), &is_hit)
	...
	request.Has_shown_uids = uid_shown_map
	return 0
}
